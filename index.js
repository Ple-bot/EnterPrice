    ...
    client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
        client.getGroupMembersId(id)
            .then((ids) => {
                ...
                if (ids.length <= 10) {
                    client.sendText(id, 'Lo siento, hay un limite de 10 usuarios en este grupo para usar el bot simultaneamente~').then(() => client.leaveGroup(id))
                } else {
                    client.sendText(id, `Hola miembros del grupo *${name}*, Gracias por invitarme a el grupo, para ver el menu del BOT enviar *#menu*`)
                }
            }))
